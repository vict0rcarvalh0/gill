 ...
 */
export async function loadKeypairFromFile(filePath: string = DEFAULT_CLI_KEYPAIR_PATH): Promise<KeyPairSigner> {
  const resolvedPath = resolve(filePath.startsWith("~") ? filePath.replace("~", homedir()) : filePath);
  return createKeyPairSignerFromBytes(Uint8Array.from(JSON.parse(readFileSync(resolvedPath, "utf8"))));
}
...
/**
 * Load a `KeyPairSigner` from an environment variable
 * (i.e. those generated by the `solana-keygen` command)
 *
...
export async function loadKeypairFromEnvironment<TName extends keyof NodeJS.ProcessEnv | string>(
  ...
  return createKeyPairSignerFromBytes(Uint8Array.from(JSON.parse(process.env[variableName])));
}
...
export async function loadKeypairSignerFromEnvironment<TName extends keyof NodeJS.ProcessEnv | string>(
  ...
  return loadKeypairFromEnvironment(variableName);
}
